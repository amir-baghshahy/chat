...
export function ForwardModal() {
  const { modals, closeModal, currentChat, showToast } = useTelegram()

  if (!modals.forward) return null

  const handleForwardToChat = (chat: Chat) => {
    closeModal('forward')
    showToast('Forwarded', `Message forwarded to ${chat.name}`, 'success')
  }
    ...
    >
      ...
      >
        <div className="px-5 py-4 flex items-center justify-between border-b border-[color:var(--tg-border)]">
          <h2 className="text-[18px] font-semibold">Forward Message</h2>
          <button
          ...
          {availableChats.map(chat => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[15px] font-semibold text-[color:var(--tg-text-primary)] truncate">{chat.name}</div>
                <div className="text-[13px] text-[color:var(--tg-text-tertiary)] mt-0.5">
                  {chat.online ? 'online' : 'last seen recently'}
                </div>
              </div>
              <i className="fas fa-chevron-right text-[color:var(--tg-text-tertiary)] text-sm"></i>
            </div>
          ))}
        </div>
      </div>
