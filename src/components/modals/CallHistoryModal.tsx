...
export function CallHistoryModal() {
  const { modals, closeModal, startCall, chatsData } = useTelegram()
        ...
        <div className="flex-1 overflow-y-auto p-0 modal-scrollable">
          <div className="max-h-[500px] overflow-y-auto">
            {callHistoryData.map(call => {
              // Find the corresponding chat for this call
              const chat = chatsData.find(c => c.name === call.name)
              const handleVoiceCall = () => {
                if (chat) {
                  startCall(chat, false)
                  closeModal('callHistory')
                }
              }
                ...
                >
                  ...
                  <div className="flex gap-1.5 sm:gap-2 flex-shrink-0">
                    <button
                      ...
                      className="w-8 h-8 sm:w-9 sm:h-9 border-none bg-[color:var(--tg-blue)] text-white rounded-full cursor-pointer flex items-center justify-center text-xs sm:text-sm transition-colors hover:bg-[color:var(--tg-blue-dark)]"
                    >
